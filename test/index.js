...
test("SVG node hook is registered", function (t) {
  t.equal(typeof require.extensions[".svg"], "function", "A function is registered for handling SVG files.");
  t.end()
});

test("SVG node hook exports a function that unregisters the hook", function (t) {
  unhookSVG();
  t.equal(typeof require.extensions[".svg"], "undefined", "No function is registered for handling SVG files after calling exported function.");
  t.end();
});
